...
export function buildGraph(corridors: BrtCorridor[], halteByLeg: BrtHalte[][]): Graph {
  ...
  // Ride edges — walk each leg's ordered halte list and connect i -> i+1.
  for (const leg of halteByLeg) {
    for (let i = 0; i < leg.length; i++) {
      ...
      const id: NodeId = `${h.kor}|${h.sh_id}`
      if (!nodes.has(id)) {
        nodes.set(id, { id, kor: h.kor, sh_id: h.sh_id, sh_name: h.sh_name, lat, lng })
      }
      if (i > 0) {
        const prev = leg[i - 1]
  ...
  // Transfer edges — pair every node with every other node at the same
  // physical stop (different kor, distance < TRANSFER_RADIUS_M).
  const nodeList = [...nodes.values()]
  for (let i = 0; i < nodeList.length; i++) {
    const a = nodeList[i]
    for (let j = i + 1; j < nodeList.length; j++) {
      const b = nodeList[j]
      if (a.kor === b.kor) continue
      const distM = haversineMeters(a, b)
      if (distM > TRANSFER_RADIUS_M) continue
      addEdge(a.id, { to: b.id, weight: TRANSFER_WAIT_MIN, distM: 0, kind: 'transfer' })
      addEdge(b.id, { to: a.id, weight: TRANSFER_WAIT_MIN, distM: 0, kind: 'transfer' })
    }
  }

  // Suppress lint complaint — corridors arg is reserved for future
  // refinements (per-corridor headway, operating-hours filter) and
...
/** Find candidate halte within WALK_RADIUS_M of a point. Returns nodes
 *  sorted by walk distance ascending. */
function nearbyHalte(graph: Graph, point: LatLng, radiusM = WALK_RADIUS_M): { node: HalteNode; distM: number }[] {
...
function shortestPath(
  ...
  const originHalte = nearbyHalte(graph, origin)
  const destHalte = nearbyHalte(graph, dest)
